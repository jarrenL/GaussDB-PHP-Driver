...
temporary_directory=$(mktemp -d /tmp/gaussdb-client.XXXXXX)
cleanup() {
...
mini_package=$(find_one "$temporary_directory/top" "GaussDB ARM64 mini driver package" \
    -name 'DBS-GaussDB-driver_*.mini.aarch64.*.tar.gz')
...
libpq_wrapper=$(find_one "$temporary_directory/catalog" "Euler2.10 ARM64 Distributed libpq wrapper" \
    ...
    -name 'GaussDB-Kernel_*_Libpq_ARM_Distributed.tar.gz')
...
actual_libpq_sha256=$(verify_sha256_if_expected "$temporary_directory/client/lib/libpq.so.5.5" "${GAUSSDB_EXPECTED_LIBPQ_SHA256:-}")

replace_directory "$temporary_directory/client" "$output_directory"

echo "Extracted GaussDB Distributed ARM64 client to: $output_directory"
