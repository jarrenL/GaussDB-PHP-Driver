...
| 平台 | 接入实现 | 计划交付物 |
|---|---|---|
| Linux ARM64 | PHP PDO 扩展 + GaussDB 507 ARM64 libpq | `pdo_gaussdb.so`、客户端库包、安装脚本/镜像 |
| Linux x86_64 | 同一扩展源码 + GaussDB 507 x86_64 libpq | `pdo_gaussdb.so`、客户端库包、安装脚本/镜像 |
| Windows x64/x86 | GaussDB 507 ODBC + PHP PDO_ODBC | ODBC 安装与 DSN 脚本、PHP 配置、公共契约测试 |
...
- Windows PHP：8.3.8 NTS x64，启用官方 `PDO_ODBC`
...
Windows x64 已在 UTM Windows 11（AMD64）中完成真实验证：507 X64 安装器可用，驱动注册名为 `GaussDB Unicode`，PHP 8.3.8 x64 的 `PDO_ODBC` 可通过无 DSN 连接串连接本地 M 模式实例。CRUD、预处理、DECIMAL、BOOLEAN、TIMESTAMP 和事务回滚通过。Windows x86 安装器已完成提取和校验，但尚未在 32 位 PHP 环境执行连库测试。
...
- 事务回滚和基础列元数据。
- Windows x64 PDO_ODBC 连接、建表、参数化写入、查询和事务回滚。
...
在独立 `gaussdb:` DSN 完成前，原型继续使用 `pgsql:` DSN 加载 GaussDB libpq。
