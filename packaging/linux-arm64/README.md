...
目标客户端包：与服务端匹配的 GaussDB Distributed / ARM64 libpq。
...
提取脚本从官方总包中定位 Distributed/ARM64 客户端并记录 `libpq.so.5.5` SHA-256。生产交付应显式设置 `GAUSSDB_EXPECTED_LIBPQ_SHA256`，使用厂商公布或客户验收的期望值进行强校验。生成的 `build/` 目录不会提交到 Git。
...
PDO 扩展直接使用配套 GaussDB 头文件和 `libpq.so.5.5` 编译、链接。构建阶段会比较 `pdo_pgsql.so`/`pgsql.so` 所需的 `PQ*` 符号与 GaussDB libpq 的导出符号，缺失时立即失败。运行时通过扩展和客户端库自身的 RPATH 定位私有依赖，不设置全局 `LD_LIBRARY_PATH`，因此客户端随包的 SSL/Kerberos 库不会影响无关 PHP 扩展。
